...
NNMinCoeffs = TypedDict("NNMinCoeffs",
                         {"IHW": NDArray[np.float64],
                          "HB" : NDArray[np.float64], 
                          "HOW": NDArray[np.float64],
...
class NNetworkMinimal:
    ...
    def gradient_descent(self, data: NDArray[np.float64], labels: NDArray[np.float64]) -> None:
        ...
        for i in range(self.__maxiter):
            ...
            #    10 x N                                      10 x N
            # print(Out.mean())
            out_hat = softmax(out)      # WARNING :: NAN HOSTSPOT (if the pixel data is not normalized)
            ...
            #    10 x 1                           10 x 1       
            self.__dB: NDArray[np.float64] = d_hidden.sum(axis = 1).reshape(10, 1) / labels.size
    ...
    def save(self, filepath: str) -> None:
        ...
        if filepath.count('.'):
            raise ValueError(f"Argument 'filepath' isn't allowed to contain periods! {filepath}[{filepath.find('.')}]")
    ...
    def load(self, filepath: str) -> None:
        ...
        if not filepath.endswith(".nnm"):
            TypeError("Only models an .nnm extension, serialized using the .save() method are supported!")
