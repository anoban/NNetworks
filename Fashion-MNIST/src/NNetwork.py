...
# an extended version of the NNetworkMinimal class, that allows dynamic introduction of multiple hidden layers.
class NNetworkExtended:
    """
    A class representing a bare minimum neural network with one input layer, one hidden layer and an output layer
    """

    def __init__(self, nodes_in: int, nlayers_hidden: int, nodes_hid: list[int], nodes_out: int, alpha: float = 0.1, maxiterations: int = 500) -> None:
        """
        Parameters:
        nodes_in: np.uint64 - number of nodes in the input layer
        nlayers_hidden: int - 
        ...
        assert(nlayers_hidden == len(nodes_hid))   # make sure each hidden layer has a number of nodes specified.
