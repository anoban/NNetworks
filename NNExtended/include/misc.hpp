...
template<typename _Ty0, typename _Ty1> struct is_in final {
        static constexpr bool value { std::is_same<_Ty0, _Ty1>::value };
};

template<typename _Ty0, typename... _TyList> struct is_in final {
        static constexpr bool value { std::is_same<typename std::remove_cv<_Ty0>::type, _Ty1>::value ||
